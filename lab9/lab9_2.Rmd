...
homerange %>%
  ...
  summarize(min_log=min(log10.mass),
            max_log=max(log10.mass),
            mean_log=mean(log10.mass))
  
```

```{r}
homerange %>%
    ggplot(aes(x=trophic.guild, y=log10.mass)) + geom_col()
```
...
homerange %>%
  ...
  ggplot(aes(y=family, x=log10.mass))+geom_col()
```
...
homerange %>%
  filter(trophic.guild == "carnivore", class== "mammalia") %>%
  ggplot(aes(y=family, x=log10.mass))+geom_boxplot()+labs(title="Family vs. Log10.Mass")
