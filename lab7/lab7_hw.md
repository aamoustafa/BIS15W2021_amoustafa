...
There are 0 NA's in this data set, which makes sense since NA's are being represented as "-999" and "30258.711"
...
There are 170691 NA's in this data set, which makes sense since NA's are being represented as NA's. 
...
This result makes sense biologically for mammals, as they do not lay eggs and the NA's are interpreted as not applicable. For reptiles and birds, the NA's are interpreted as missing data, although there is a very high percentage of missing data which doesn't make sense.  
...
The NA's represent no's, in which whether or not a species is terrestrial for example is represented with either a 1 or an NA. If assigned an NA, that means the species is not terrestrial. If assigned 1, then the species is terrestrial. 
