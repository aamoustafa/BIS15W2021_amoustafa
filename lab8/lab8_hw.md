...
```r
sydneybeaches_long_tidy <-sydneybeaches_long %>%
  separate(date, into = c("month","day","year"), sep = "/") 
sydneybeaches_long_tidy
```
...
```r
avg_enterococci <- sydneybeaches_long_tidy %>%
  group_by(year, site) %>%
...
10. Please complete the class project survey at: BIS 15L Group Project
